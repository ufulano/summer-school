  ...
  <div class="page-wrapper">
    <el-card class="toolbar" shadow="never">
      <div class="toolbar-row">
        <el-input v-model="filters.orderId" placeholder="订单ID" clearable style="max-width: 240px" @keyup.enter.native="fetchList" />
    ...
    <el-card class="table-card" shadow="never">
      ...
      <el-table v-else v-loading="loading" :data="tableData" size="small" border style="width: 100%" empty-text="暂无数据">
        ...
        <el-table-column prop="ragName" label="知识库名称" min-width="160" />
        <el-table-column prop="knowledgeTag" label="知识标签" min-width="140" />
    ...
    <el-dialog v-model="formVisible" :title="formTitle" width="560px" destroy-on-close>
      <el-form :model="form" label-width="100px">
        <el-form-item label="知识库名称">
          <el-input v-model="form.ragName" />
        </el-form-item>
        <el-form-item label="知识标签">
          <el-input v-model="form.knowledgeTag" />
        </el-form-item>
...
const formTitle = computed(() => (form.value.id ? '编辑RAG订单' : '新增RAG订单'))
const saving = ref(false)
const form = ref({ id: undefined, ragName: '', knowledgeTag: '', status: 1 })

const onSearch = () => { pageNum.value = 1; fetchList() }
const resetAndFetch = () => { filters.orderId = ''; pageNum.value = 1; fetchList() }
...
const fetchList = async () => {
  ...
  error.value = false
  try {
    const params = { pageNum: pageNum.value, pageSize: pageSize.value, orderId: filters.orderId || undefined }
    const { data } = await api.post(ENDPOINTS.RAG.LIST, params)
...
const openForm = async (row) => {
  if (row && row.id) {
    try { const { data } = await api.get(`${ENDPOINTS.RAG.DETAIL}?id=${row.id}`); form.value = { id: data.id, ragName: data.ragName, knowledgeTag: data.knowledgeTag, status: data.status } }
    catch (e) { ElMessage.error('获取详情失败'); return }
  } else { form.value = { id: undefined, ragName: '', knowledgeTag: '', status: 1 } }
  formVisible.value = true
}

const submit = async () => {
  if (!form.value.ragName) { ElMessage.warning('请输入知识库名称'); return }
  saving.value = true
  try {
    const payload = { ragName: form.value.ragName, knowledgeTag: form.value.knowledgeTag, status: form.value.status }
    if (form.value.id) { await api.post(ENDPOINTS.RAG.UPDATE, { ...payload, id: form.value.id }); ElMessage.success('更新成功') }
    else { await api.post(ENDPOINTS.RAG.ADD, payload); ElMessage.success('新增成功') }
    formVisible.value = false; fetchList()
  } catch (e) { ElMessage.error('保存失败') } finally { saving.value = false }
}

const confirmDelete = async (row) => {
  try { await ElMessageBox.confirm(`确定要删除订单 ID ${row.id} 吗？`, '确认删除', { type: 'warning' }); await api.get(`${ENDPOINTS.RAG.DELETE}?id=${row.id}`); ElMessage.success('删除成功'); fetchList() } catch (e) { }
}
